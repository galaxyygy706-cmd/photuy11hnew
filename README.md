...
```
Rules hiện cho phép đọc/ghi không cần đăng nhập (app chưa gắn Firebase Authentication) nhưng siết theo từng field. Khi gắn đăng nhập nhân viên thì thêm `request.auth != null` vào từng điều kiện.

> ⚠️ **Nếu sửa rule của `tables`, phải giữ `paidAt` trong danh sách `hasOnly`.** App ghi `status` và `paidAt` trong **cùng một lệnh** ở hai chỗ: thanh toán xong (`cho_don` + mốc thời gian) và dọn xong (`trong` + `null`). Rule chỉ cho `hasOnly(['status'])` sẽ từ chối cả lệnh — mà Firestore đã áp dụng lạc quan vào cache local trước rồi mới rollback, nên trên máy nhân viên bàn chỉ *nháy* qua "Chờ dọn" rồi quay về trạng thái cũ, trông y như app bị lỗi logic.

Rule của `tables` cũng chặn `create`, nên `seedTablesIfEmpty()` trong `app.js` không tạo được bàn lần đầu. Chạy lần đầu thì tạm đổi thành `allow create: if true;`, để app tự sinh 13 bàn, rồi khóa lại.

Nếu sau này thêm field mới vào doc `tables`, nhớ bổ sung tên field đó vào `hasOnly([...])`, nếu không lệnh ghi sẽ bị từ chối lặng lẽ giống hệt trường hợp `paidAt`.
